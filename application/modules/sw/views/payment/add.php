...
$(document).ajaxComplete(function() {
	...
	var pb_less_amt = 0.00;
		...
		$('#stotamt').html((st-(sb_less_amt)).toFixed());
		$('#ptotamt').html((pt-(sb_less_amt-(sb_less_amt*.01))).toFixed());
	})
})	
$(document).ready(function(){
	$('#cal').click(function(){
	...
	$('.p_bill_amt').each(function(){
		...
		$('#ptotamt').html((pbamt-(sb_less_amt-(sb_less_amt*.01))).toFixed());
		...
		if(dist_cd > 0 ){
			...
			.done(function(data){
				...
				var newElement1= '<tr>'
								...
								+'</td>'
								+'<td>'
									+'<input type="date" name="s_bill_dt[]" class="s_bill_dt" id="s_bill_dt" style="width:100px" readonly>'
								+'</td>'
								+'<td>'
