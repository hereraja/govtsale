	...
	<div class="col-md-11 container form-wraper" style="margin-left: 0%;">
		<form method="POST" id="form" role="form" name="add_form" action="<?php echo site_url("sw/savemr");?>" onsubmit="return validate()" >
				...
				<div class="form-group row" >
					...
					<tr>
					<th style="width:100px">sale bill no</th><th>sale bill dt</th>
					<th>sale cgst</th><th>sale Sgst</th><th>Sale bill amt</th>
					<th>Pur bill no</th><th>Pur bill dt</th><th> cgst</th><th> Sgst</th><th>Pur bill amt</th>
					<th>
					...
					<tfooter>
					<tr><td colspan="2" ></td><td colspan="2" style="text-align: right;">Total:</td><td id="st">0.00</td><td colspan="4"></td><td id="pt">0.00</td></tr>
		            <tr><td colspan="2" ></td><td colspan="2" style="text-align: right;">Less Amount:</td><td><input type="text" id="sb_less_amt" class="form-control sb_less_amt" name="sb_less_amt" value="" >
				    </td><td colspan="4"><td><input type="text" class="form-control pb_less_amt" name="pb_less_amt" value="<?php if(isset($bill_detail->pb_less_amt)) echo $bill_detail->pb_less_amt ;?>"></td></tr>
	                <tr><td colspan="2" ></td><td colspan="2" style="text-align: right;">Total:</td><td id="stotamt"></td><td colspan="4"></td><td id="ptotamt"></td></tr>
					</tfooter>		
					</table>
...
$(document).ready(function(){
	...
	$("#addrow1").click(function()
	{
		...
		if(dist_cd > 0 ){
			...
			$.get('<?php echo site_url("sw/js_get_sbno_pervendor") ?>', {vendor_id: $('#vendor_id').val()})
			.done(function(data){
				...
				var newElement1= '<tr>'
								+'<td id= "s_bill_no" >'
									+'<select name="s_bill_no[]" id="s_bill_no" class= "form-control s_bill_no" required>'
										+string
									+'</select>'
								+'</td>'
								+'<td>'
									+'<input type="date" name="s_bill_dt[]" class="s_bill_dt" id="s_bill_dt" style="width:131px" readonly>'
								+'</td>'
								+'<td>'
								...
								+'</td>'
								+'<td>'
									+'<input type="date" name="p_bill_dt[]" class="p_bill_dt" id="p_bill_dt"  readonly>'
								+'</td>'
								+'<td>'
...
var s_bill_no = $(this).val();
	$.get('<?php echo site_url("sw/js_get_sbpb_detail") ?>',{s_bill_no:s_bill_no})
	.done(function(data){

		var value = JSON.parse(data);
		row.find("td:eq(1) input[type='date']").val(value.s_bill_dt);
		row.find("td:eq(2) input[type='text']").val(value.s_cgst);
		row.find("td:eq(3) input[type='text']").val(value.s_sgst);
		row.find("td:eq(4) input[type='text']").val(value.s_bill_amt);
		row.find("td:eq(5) input[type='text']").val(value.p_bill_no);
		row.find("td:eq(6) input[type='date']").val(value.p_bill_dt);
		row.find("td:eq(7) input[type='text']").val(value.p_cgst);
		row.find("td:eq(8) input[type='text']").val(value.p_sgst);
		row.find("td:eq(9) input[type='text']").val(value.p_bill_amt);


		$('#st').html(st+parseFloat(value.s_bill_amt));
		$('#pt').html(pt+parseFloat(value.p_bill_amt));
	})
