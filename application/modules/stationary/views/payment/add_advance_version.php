<div class="wraper">      
	<div class="col-md-12 container form-wraper" style="margin-left: 0%;">
		<form method="POST" id="form" role="form" name="add_form" action="<?php echo site_url("stationary/savemr_adv_ver");?>"  >
				...
				<div class="form-group row">
					...
					<option value="">Select Customer</option>
								...
								?>
										<option value="<?php echo $pro->project_cd; ?>" ><?php echo $pro->name; ?></option>
								<?php
									} 
								?>
				    </select>
				...
				<div class="form-group row">
				   <label for="trans_dt" class="col-sm-1 col-form-label">Sale Bill No<font color="red">*</font></label>
					<div class="col-sm-3">
					<select name="bulk_trans_id" id="bulk_trans_id" class="form-control" required >
					    <option value="">Select Bill NO</option>
						
				    </select>
					</div>
				    <label for="trans_dt" class="col-sm-1 col-form-label">Vendor Payment Bank<font color="red">*</font></label>
					<div class="col-sm-3">
					<select name="debit_bank" id="debit_bank" class="form-control" required >
				...
				<div class="form-group row" >
					...
					<!-- NOTE: Please click on CAL button after selecting all bill. -->
					<span>
				...
				<div class="form-group row">
				<label for="trans_dt" class="col-sm-1 col-form-label">Payment Received Bank<font color="red">*</font></label>
					<div class="col-sm-3">
...
$(document).ready(function(){
    ...
    $("#addrow").click(function()
    {
		...
		if($('#mr_counter').val() < 30) {
					y = y + 1;
					$('#mr_counter').val(y);
					$("#intro").append($(newElement));
				}else{
					alert('You have already added 30 row');
				} 
    });  
...
$('#bulk_trans_id').change(function(){

    var bulk_trans_id = $(this).val();
	var sbamt = 0.00;var pbamt = 0.00;
    $.get('<?php echo site_url("stationary/js_get_salebill_detail");?>',{ bulk_trans_id: bulk_trans_id })
    .done(function(data)
    {
	    var string = '';
		...
		$.each(JSON.parse(data), function( index, value ) {
			...
			string += '<tr><td>'+value.s_bill_no+'</td><td>'+value.s_bill_dt+'</td><td>'+value.s_taxable_value+'</td><td>'+value.s_cgst+'</td><td>'+value.s_sgst+'</td><td>'+value.s_bill_amt+'</td><td>'+value.p_bill_no+'</td><td>'+value.p_bill_dt+'</td><td>'+value.p_taxable_value+'</td><td>'+value.p_cgst+'</td><td>'+value.p_sgst+'</td><td>'+value.p_bill_amt+'</td></tr>';
		...
		string +='<tr><td colspan="2" style="text-align: right;"></td><td colspan="3" style="text-align: right;">Less Amount test:</td><td><input type="text" id="sb_less_amt" class="form-control sb_less_amt" name="sb_less_amt" value="0.00" ></td><td></td><td></td><td></td><td></td><td><input type="text" class="form-control pb_less_amt" name="pb_less_amt" value="0.00"></td></tr>';
		string +='<tr><td colspan="2" style="text-align: right;"></td><td colspan="3" style="text-align: right;">Total:</td><td id="stotamt">'+sbamt+'</td><td></td><td></td><td></td><td></td><td id="ptotamt">'+pbamt+'</td></tr>';
...
var sbamt = 0.00;var pbamt = 0.00;
$.get('<?php echo site_url("stationary/js_get_unpaid_salebill");?>',{ customer_id: customer_id,vendor_id:vendor_id })
.done(function(data)
{
	//var string = '';
	var string = "<option value=''>Select Bill No</option>";
	$.each(JSON.parse(data), function( index, value ) {
		string += '<option value='+value.bulk_trans_id+'>'+value.s_bill_no+'</option>';
	});
	$('#bulk_trans_id').html('');
	$('#bulk_trans_id').html(string);
})
...
$(document).ajaxComplete(function() {
	...
	var commision_rate = <?=$vfc?>;
	 less_amt = parseFloat(((sb_less_amt*100)/commision_rate).toFixed());
	//alert(less_amt);
		$('.pb_less_amt').val(parseFloat(less_amt));
		$('#stotamt').html(st-(sb_less_amt));
		$('#ptotamt').html(pt-less_amt);
	})
})
$(document).ready(function(){
	$('#cal').click(function(){
