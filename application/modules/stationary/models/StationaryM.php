<?php defined('BASEPATH') OR exit('No direct script access allowed');
    ...
    {   $and = '';
        ...
        if($supplier_id == 0){
            $sql = $this->db->query("SELECT a.s_bill_no,SUM(a.s_taxable_value) s_taxable_value,
            SUM(a.s_cgst) s_cgst ,SUM(a.s_sgst) s_sgst,
            SUM(a.s_bill_amt) s_bill_amt,SUM(a.p_taxable_value) p_taxable_value,
            SUM(a.p_cgst) p_cgst,SUM(a.p_sgst) p_sgst,SUM(a.p_bill_amt) p_bill_amt  
            FROM td_stn_pbsb_dtls a
            WHERE a.s_bill_dt >= '$startDt' 
            AND a.s_bill_dt <= '$endDt' $and group by a.s_bill_no");
            return $sql->result();
        }else{
